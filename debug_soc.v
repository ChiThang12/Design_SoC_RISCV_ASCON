...
// ============================================================================
// [DBG-NEW] Theo dõi DCache trả stale data cho ASCON reads
// Nếu CPU load từ 0x2000xxxx nhưng DCache trả data cũ (hit) thay vì
// fetch từ ASCON slave → đây là nguyên nhân polling mãi không thấy DMA_DONE
always @(posedge clk) begin
    if (rst_n_r && dc_req && dc_ready && !dc_we) begin
        if (dc_addr[31:12] == 20'h20000) begin
            $display("[%6d] [DCACHE-ASCON] LOAD  addr=0x%08h  data=0x%08h  (ASCON_live=0x%08h)%s",
                     cycle_count, dc_addr, dc_rdata, ascon_status_word,
                     (dc_rdata !== ascon_status_word && dc_addr == 32'h20000004) ?
                     "  [!!!] STALE — DCache hit stale ASCON data!" : "");
        end
    end
end
always @(posedge clk) begin
...
always @(posedge clk) begin
    if (rst_n_r) begin
        ...
        if (m2_rvalid && m2_rready) begin
            if (`LOG_LEVEL >= 1)
                $display("[%6d] [M2-R ] DMA FETCH data  addr_prev=0x%08h  data=0x%08h  rresp=%0d%s",
                         cycle_count, m2_ar_addr_saved, m2_rdata, m2_rresp,
                         m2_rlast ? "  [LAST]" : "");
            if (m2_rlast) begin
        ...
        if (m2_wvalid && m2_wready)
            $display("[%6d] [M2-W ] DMA WRITE data  addr=0x%08h  data=0x%08h  strb=%b%s",
                     cycle_count, m2_aw_addr_saved, m2_wdata, m2_wstrb,
                     m2_wlast ? "  [LAST]" : "");
...
always @(posedge clk) begin
    if (rst_n_r) begin
        ...
        if (s2_arvalid) begin
            ...
            if (`LOG_LEVEL >= 1)
                $display("[%6d] [S2-ASCON] READ   offset=0x%03h  rdata=0x%08h  rvalid=%0d",
                         cycle_count, s2_araddr[11:0], s2_rdata, s2_rvalid);
        end
        // Log khi read data thực sự được trả về (rvalid pulse)
        if (s2_rvalid && `LOG_LEVEL >= 1)
            $display("[%6d] [S2-ASCON] RDATA  rdata=0x%08h  rresp=%0d  (STATUS_live=0x%08h  reg_dma_en=%0d)",
                     cycle_count, s2_rdata, s2_rresp,
                     ascon_status_word, ascon_reg_dma_en);
        if (s2_awvalid) begin
            s2_access_cnt = s2_access_cnt + 1;
            if (`LOG_LEVEL >= 1)
        ...
        // Log khi write được accept (bvalid)
        if (s2_bvalid && `LOG_LEVEL >= 1)
            $display("[%6d] [S2-ASCON] BRESP  bresp=%0d  STATUS_after=0x%08h  reg_dma_en=%0d  core_start=%0d  dma_start=%0d",
                     cycle_count, s2_bresp,
                     ascon_status_word, ascon_reg_dma_en,
                     ascon_core_start, ascon_dma_start);
        if (s3_arvalid) begin
...
// ============================================================================
// [DBG-NEW] Biến theo dõi polling loop
integer ascon_poll_cnt;
integer ascon_poll_last_log;
reg     ascon_polling_active;

initial begin
    ascon_poll_cnt       = 0;
    ascon_poll_last_log  = 0;
    ascon_polling_active = 0;
end
always @(posedge clk) begin
    if (!rst_n_r) begin
...
always @(posedge clk) begin
    if (rst_n_r) begin
        if (ascon_soft_rst)
            $display("[%6d] [ASCON] SOFT_RST asserted  STATUS_before=0x%08h  reg_dma_en=%0d",
                     cycle_count, ascon_status_word, ascon_reg_dma_en);

        // [DBG-NEW] Phát hiện và log polling loop
        // CPU đang poll STATUS nếu: đọc 0x20000004 liên tục
        if (s2_arvalid && s2_araddr[11:0] == 12'h004) begin
            ascon_poll_cnt = ascon_poll_cnt + 1;
            ascon_polling_active = 1;
            // Log mỗi 500 lần poll để không spam, nhưng vẫn theo dõi được tiến trình
            if (ascon_poll_cnt == 1 || (ascon_poll_cnt % 500) == 0) begin
                $display("[%6d] [ASCON-POLL] #%0d  STATUS_live=0x%08h  {dma_err=%0d dma_done=%0d dma_busy=%0d core_done=%0d core_busy=%0d}  reg_dma_en=%0d  reg_dma_src=0x%08h",
                         cycle_count, ascon_poll_cnt,
                         ascon_status_word,
                         ascon_dma_error, ascon_dma_done_st, ascon_dma_busy,
                         ascon_core_done, ascon_core_busy,
                         ascon_reg_dma_en, ascon_dma_src_r);
            end
        end else if (ascon_polling_active && !s2_arvalid) begin
            // Thoát khỏi polling — log lý do
            if (ascon_poll_cnt > 0)
                $display("[%6d] [ASCON-POLL] EXIT after %0d polls  STATUS=0x%08h",
                         cycle_count, ascon_poll_cnt, ascon_status_word);
            ascon_polling_active = 0;
        end

        if (ascon_core_start) begin
            ascon_start_cnt = ascon_start_cnt + 1;
            $display("[%6d] [ASCON] CORE START  #%0d  dma_en=%0d  reg_dma_src=0x%08h  reg_dma_dst=0x%08h  reg_dma_len=%0d  STATUS=0x%08h",
        ...
        if (ascon_dma_start) begin
            ...
            $display("[%6d] [ASCON] DMA  START  #%0d  src=0x%08h  dst=0x%08h  len=%0d  reg_dma_en=%0d",
...
task print_report;
    ...
    begin
        ...
        // [DBG-NEW] Dump đầy đủ ASCON internal register state khi halt
        $display("|  -- ASCON INTERNAL REGS AT HALT --");
        $display("|  reg_mode    = 0x%02h  reg_irq_en = 0x%02h  reg_dma_en = %0d",
                 soc.u_ascon.u_slave.reg_mode,
                 soc.u_ascon.u_slave.reg_irq_en,
                 soc.u_ascon.u_slave.reg_dma_en);
        $display("|  reg_key     = %08h_%08h_%08h_%08h",
                 soc.u_ascon.u_slave.reg_key_0,
                 soc.u_ascon.u_slave.reg_key_1,
                 soc.u_ascon.u_slave.reg_key_2,
                 soc.u_ascon.u_slave.reg_key_3);
        $display("|  reg_nonce   = %08h_%08h_%08h_%08h",
                 soc.u_ascon.u_slave.reg_nonce_0,
                 soc.u_ascon.u_slave.reg_nonce_1,
                 soc.u_ascon.u_slave.reg_nonce_2,
                 soc.u_ascon.u_slave.reg_nonce_3);
        $display("|  reg_dma_src = 0x%08h  reg_dma_dst = 0x%08h  reg_dma_len = %0d",
                 soc.u_ascon.u_slave.reg_dma_src,
                 soc.u_ascon.u_slave.reg_dma_dst,
                 soc.u_ascon.u_slave.reg_dma_len);
        $display("|  wr_state    = %0d  rd_state = %0d",
                 soc.u_ascon.u_slave.wr_state,
                 soc.u_ascon.u_slave.rd_state);
        $display("|  status_done=%0d  status_dma_done=%0d  status_err=%0d  status_dma_err=%0d",
                 soc.u_ascon.u_slave.status_done,
                 soc.u_ascon.u_slave.status_dma_done,
                 soc.u_ascon.u_slave.status_error,
                 soc.u_ascon.u_slave.status_dma_error);
        $display("|  ascon_poll_cnt = %0d  (lần CPU đọc STATUS)", ascon_poll_cnt);
        ...
        // [DBG-NEW] ASCON output snapshot: CTEXT và TAG từ DMEM
        $display("");
        $display("+--- (5b) ASCON CTEXT/TAG OUTPUT (từ DMEM sau DMA) -------------+");
        begin : ascon_out_blk
            integer ci;
            reg [31:0] ctext_base, tag_base, wval;
            reg found_c;
            ctext_base = 32'h10000010;
            tag_base   = 32'h10000020;
            $display("|  CTEXT @ 0x10000010:");
            $write("|    ");
            for (ci = 0; ci < 2; ci = ci + 1) begin
                wval = 32'h0; found_c = 0;
                begin : sb_scan_c
                    integer si;
                    for (si = 0; si < sb_cnt; si = si + 1)
                        if (sb_addr[si] === ctext_base + ci*4) begin
                            wval = sb_data[si]; found_c = 1;
                        end
                end
                $write("0x%08h%s ", wval, found_c ? "" : "(?)");
            end
            $display("");
            $display("|  TAG   @ 0x10000020:");
            $write("|    ");
            for (ci = 0; ci < 4; ci = ci + 1) begin
                ...
                begin : sb_scan_t
                    integer si;
                    for (si = 0; si < sb_cnt; si = si + 1)
                        if (sb_addr[si] === tag_base + ci*4) begin
                            wval = sb_data[si]; found_c = 1;
                        end
                end
                $write("0x%08h%s ", wval, found_c ? "" : "(?)");
...
task print_banner;
    begin
        $display("");
        $display("+=================================================================+");
        $display("|   RISC-V SoC  +  ASCON IP  --  Debug Testbench  v5.1-DBG      |");
