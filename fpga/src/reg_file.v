...
module reg_file (
    ...
    // ========================================================================
    // READ: combinational with a WB bypass. ID can sample a register during
    // the same cycle that WB is about to commit it at negedge; without this
    // bypass, a load followed by JALR/CALL captures the previous value.
    // ========================================================================
    wire wb_hit_rs1 = regwrite && (write_reg != 5'b00000) &&
                      (write_reg == read_reg_num1);
    wire wb_hit_rs2 = regwrite && (write_reg != 5'b00000) &&
                      (write_reg == read_reg_num2);

    assign read_data1 = (read_reg_num1 == 5'b00000) ? 32'h00000000 :
                        wb_hit_rs1 ? write_data : registers[read_reg_num1];

    assign read_data2 = (read_reg_num2 == 5'b00000) ? 32'h00000000 :
                        wb_hit_rs2 ? write_data : registers[read_reg_num2];
