...
module PIPELINE_REG_EX_MEM (
    ...
    always @(posedge clock or posedge reset) begin
        if (reset) begin
            ...
            // Control path: while one upstream stall episode is active, let each
            // distinct EX instruction advance once, then bubble duplicate replays.
            // Do not keep the signature after the stall clears: ISR/trap code can
            // execute the same PC again later, and that is a new dynamic instruction.
