...
module hazard_detection (
    ...
    // MUL result stall: keep a dependent instruction out of EX until the MUL
    // reaches WB, where mul_result_direct can be forwarded safely. The MEM
    // stage carries alu_result=0 for MUL, so forwarding from MEM is invalid.
    wire mul_result_stall;
    assign mul_result_stall =
        (mul_in_ex &&
         (rd_id_ex != 5'b0) &&
        ...
        (mul_in_mem_stage &&
         (rd_mem_stage != 5'b0) &&
         ((rs1_used_id && (rs1_id != 5'b0) && (rs1_id == rd_mem_stage)) ||
          (rs2_used_id && (rs2_id != 5'b0) && (rs2_id == rd_mem_stage))));
    ...
    assign stall    = load_use_hazard || lsu_dep_stall ||
