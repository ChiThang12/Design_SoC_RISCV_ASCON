...
module PIPELINE_REG_ID_EX (
    ...
    input wire        lsu_fwd_a_valid,
    input wire        lsu_fwd_b_valid,
    input wire [31:0] lsu_fwd_data,
    input wire        fwd_a_self_replay,
    input wire        fwd_b_self_replay,
    ...
    always @(posedge clock or posedge reset) begin
        if (reset) begin
            ...
            // Only update operand registers that are currently being forwarded.
            if (lsu_fwd_a_valid)
                read_data1_out <= lsu_fwd_data;
            else if ((fwd_a_sel != 2'b00) && !fwd_a_self_replay)
                read_data1_out <= fwd_a_data;
            if (lsu_fwd_b_valid)
                read_data2_out <= lsu_fwd_data;
            else if ((fwd_b_sel != 2'b00) && !fwd_b_self_replay)
                read_data2_out <= fwd_b_data;
        end
    end
