...
module LSU (
    ...
    // =========================================================================
    integer fi;
    reg [SB_BITS-1:0] fi_idx;
    always @(*) begin
        fwd_hit_r  = 1'b0;
        fwd_data_r = 32'h0;
        fwd_strb_r = 4'b0000;
        for (fi = 0; fi < SB_DEPTH; fi = fi + 1) begin
            fi_idx = sb_rd_ptr + fi[SB_BITS-1:0];
            if (sb_valid[fi_idx] && (sb_addr[fi_idx][31:2] == req_addr[31:2])) begin
                fwd_hit_r = 1'b1;
                // Merge theo thứ tự FIFO: entry trẻ hơn ghi đè entry già hơn,
                // kể cả khi store-buffer đã wrap-around.
                if (sb_wstrb[fi_idx][0]) begin fwd_data_r[7:0]   = sb_wdata[fi_idx][7:0];   fwd_strb_r[0] = 1'b1; end
                if (sb_wstrb[fi_idx][1]) begin fwd_data_r[15:8]  = sb_wdata[fi_idx][15:8];  fwd_strb_r[1] = 1'b1; end
                if (sb_wstrb[fi_idx][2]) begin fwd_data_r[23:16] = sb_wdata[fi_idx][23:16]; fwd_strb_r[2] = 1'b1; end
                if (sb_wstrb[fi_idx][3]) begin fwd_data_r[31:24] = sb_wdata[fi_idx][31:24]; fwd_strb_r[3] = 1'b1; end
            end
        end
    end
    ...
    // A load can sit in LQ while an older store reaches SB later. Re-check
    // forwarding at dequeue time so store->load RAW is closed even across the
    // LSU queue boundary. If the match is partial or MMIO, wait for drain.
    integer hi;
    reg [SB_BITS-1:0] hi_idx;
    reg        head_fwd_any_r;
    reg        head_fwd_hit_r;
    reg [31:0] head_fwd_data_r;
    reg [3:0]  head_fwd_strb_r;
    always @(*) begin
        head_fwd_any_r  = 1'b0;
        head_fwd_hit_r  = 1'b0;
        head_fwd_data_r = 32'h0;
        head_fwd_strb_r = 4'b0000;
        for (hi = 0; hi < SB_DEPTH; hi = hi + 1) begin
            hi_idx = sb_rd_ptr + hi[SB_BITS-1:0];
            if (sb_valid[hi_idx] && (sb_addr[hi_idx][31:2] == lq_addr[lq_rd_ptr][31:2])) begin
                head_fwd_any_r = 1'b1;
                if (sb_wstrb[hi_idx][0]) begin head_fwd_data_r[7:0]   = sb_wdata[hi_idx][7:0];   head_fwd_strb_r[0] = 1'b1; end
                if (sb_wstrb[hi_idx][1]) begin head_fwd_data_r[15:8]  = sb_wdata[hi_idx][15:8];  head_fwd_strb_r[1] = 1'b1; end
                if (sb_wstrb[hi_idx][2]) begin head_fwd_data_r[23:16] = sb_wdata[hi_idx][23:16]; head_fwd_strb_r[2] = 1'b1; end
                if (sb_wstrb[hi_idx][3]) begin head_fwd_data_r[31:24] = sb_wdata[hi_idx][31:24]; head_fwd_strb_r[3] = 1'b1; end
            end
        end
        head_fwd_hit_r = head_fwd_any_r &&
                         (head_fwd_strb_r == 4'b1111) &&
                         (lq_addr[lq_rd_ptr][31:29] == 3'b000);
    end
    ...
    wire do_load_dequeue   = !lq_empty && load_fsm_ready && !fence
                          && (drain_state == DRAIN_IDLE)
                          && (lq_fwd[lq_rd_ptr] || head_fwd_hit_r || !head_fwd_any_r);
    ...
    always @(posedge clk or posedge rst) begin
        if (rst) begin
            ...
            case (load_state)
                LOAD_IDLE: begin
                    if (do_load_dequeue) begin
                        if (lq_fwd[lq_rd_ptr] || head_fwd_hit_r) begin
                            result_valid <= 1'b1;
                            result_rd    <= lq_rd[lq_rd_ptr];
                            result_data  <= apply_funct3(
                                               head_fwd_hit_r ? head_fwd_data_r
