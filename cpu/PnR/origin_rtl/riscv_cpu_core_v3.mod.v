// ============================================================================
// riscv_cpu_core.v - Optimized for Synthesis & Timing (100MHz)
// Applied: Asynchronous Reset Fixed & Register Replication (Fanout Split)
// ============================================================================
...
module riscv_cpu_core (
    ...
    always @(posedge clk or posedge rst) begin
        if (rst) begin
            ...
            if (irq_pending)
                ...
                irq_pending_lat <= irq_pending_lat;
        end
    end
    ...
    assign irq_flush_done = irq_flush_done_r;
    ...
    wire regwrite_id, alusrc_id, memread_id, memwrite_id, memtoreg_id, branch_id, jump_id;
    wire [1:0] byte_size_id;
    wire [31:0] read_data1_id, read_data2_id, imm_id;

    wire regwrite_ex, alusrc_ex, memread_ex, memwrite_ex, memtoreg_ex, branch_ex, jump_ex;
    ...
    wire [31:0] alu_in1, alu_in2, alu_in2_pre_mux, alu_result_ex;
    wire zero_flag_ex, less_than_ex, less_than_u_ex, branch_taken_ex, pc_src_ex;
    wire [31:0] target_pc_ex, pc_plus_4_ex;
    ...
    wire lsu_req_valid, lsu_req_ready, lsu_result_valid, lsu_result_ack;
    wire [3:0] lsu_req_wstrb;
    wire [31:0] lsu_result_data, lsu_scoreboard;
    wire [4:0] lsu_result_rd;

    wire regwrite_wb, memtoreg_wb, jump_wb;
    wire [31:0] alu_result_wb, mem_data_wb, pc_plus_4_wb, write_back_data_wb;
    wire [4:0] rd_wb;

    wire [1:0] forward_a, forward_b;
    wire stall, stall_if, stall_any, flush_if_id, flush_id_ex;
    ...
    IFU instruction_fetch (
        .clock(clk), .reset(rst), .pc_src(pc_src_ex), .stall(stall_any),
        .target_pc(target_pc_ex), .imem_addr(imem_addr), .imem_valid(imem_valid),
        .imem_rdata(imem_rdata), .imem_ready(imem_ready), .PC_out(pc_if), .Instruction_Code(instr_if)
    );

    PIPELINE_REG_IF_ID if_id_reg (
        .clock(clk), .reset(rst), .flush(flush_if_id_final), .stall(stall_any),
        .instr_in(instr_if), .pc_in(pc_if), .instr_out(instr_id), .pc_out(pc_id)
    );
    ...
    control control_unit (
        ...
        .alu_control(alu_control_id), .regwrite(regwrite_id), .alusrc(alusrc_id),
        .memread(memread_id), .memwrite(memwrite_id), .memtoreg(memtoreg_id),
        .branch(branch_id), .jump(jump_id), .aluop(), .byte_size(byte_size_id)
    );

    reg_file register_file (
        .clock(clk), .reset(rst), .read_reg_num1(rs1_id), .read_reg_num2(rs2_id),
        .read_data1(read_data1_id), .read_data2(read_data2_id), .regwrite(regwrite_wb),
        .write_reg(rd_wb), .write_data(write_back_data_wb)
    );

    imm_gen immediate_generator (.instr(instr_id), .imm(imm_id));

    // ========================================================================
    // ID/EX PIPELINE REGISTER (Replicated alusrc_id_ex)
    // ========================================================================
    reg regwrite_id_ex, memread_id_ex, memwrite_id_ex;
    // REPLICATION: alusrc_id_ex split into 2 to reduce fanout
    reg alusrc_id_ex_rep1, alusrc_id_ex_rep2;
    ...
    always @(posedge clk or posedge rst) begin
        if (rst) begin
            regwrite_id_ex <= 1'b0; alusrc_id_ex_rep1 <= 1'b0; alusrc_id_ex_rep2 <= 1'b0;
            memread_id_ex <= 1'b0; memwrite_id_ex <= 1'b0; memtoreg_id_ex <= 1'b0; 
            branch_id_ex <= 1'b0; jump_id_ex <= 1'b0; read_data1_id_ex <= 32'h0; 
            read_data2_id_ex <= 32'h0; imm_id_ex <= 32'h0; pc_id_ex <= 32'h0; 
            rs1_id_ex <= 5'b0; rs2_id_ex <= 5'b0; rd_id_ex <= 5'b0; 
            funct3_id_ex <= 3'b0; funct7_id_ex <= 7'b0; 
            alu_control_id_ex <= 4'b0; byte_size_id_ex <= 2'b0; opcode_id_ex <= 7'b0;
        end else begin
            if (flush_id_ex_final) begin
                regwrite_id_ex <= 1'b0; alusrc_id_ex_rep1 <= 1'b0; alusrc_id_ex_rep2 <= 1'b0;
                memread_id_ex <= 1'b0; memwrite_id_ex <= 1'b0; memtoreg_id_ex <= 1'b0; 
                branch_id_ex <= 1'b0; jump_id_ex <= 1'b0; read_data1_id_ex <= 32'h0; 
                read_data2_id_ex <= 32'h0; imm_id_ex <= 32'h0; pc_id_ex <= 32'h0; 
                ...
                alusrc_id_ex_rep1 <= alusrc_id; 
                alusrc_id_ex_rep2 <= alusrc_id;
                ...
                pc_id_ex <= pc_id; rs1_id_ex <= rs1_id; rs2_id_ex <= rs2_id;
                rd_id_ex <= rd_id; funct3_id_ex <= funct3_id; funct7_id_ex <= funct7_id;
    ...
    // Use rep1 for the generic alusrc_ex wire
    assign alusrc_ex = alusrc_id_ex_rep1;
    ...
    //wire [31:0] alu_in1_forwarded, alu_in2_pre_mux, jalr_target;
    wire [31:0] alu_in1_forwarded, jalr_target;

    assign alu_in1_forwarded = (forward_a == 2'b10) ? alu_result_mem :
                               (forward_a == 2'b01) ? write_back_data_wb : read_data1_ex;

    assign alu_in1 = (opcode_ex == 7'b0110111) ? 32'h0 :
                     (opcode_ex == 7'b0010111) ? pc_ex  : alu_in1_forwarded;

    assign alu_in2_pre_mux = (forward_b == 2'b10) ? alu_result_mem :
                             (forward_b == 2'b01) ? write_back_data_wb : read_data2_ex;

    // REPLICATION USE: alu_in2 split across two reps to reduce fanout delay
    assign alu_in2[15:0]  = alusrc_id_ex_rep1 ? imm_ex[15:0]  : alu_in2_pre_mux[15:0];
    assign alu_in2[31:16] = alusrc_id_ex_rep2 ? imm_ex[31:16] : alu_in2_pre_mux[31:16];

    alu arithmetic_logic_unit (
        .in1(alu_in1), .in2(alu_in2), .alu_control(alu_control_ex), .alu_result(alu_result_ex),
        .zero_flag(zero_flag_ex), .less_than(less_than_ex), .less_than_u(less_than_u_ex)
    );

    branch_logic branch_unit (
        .branch(branch_ex), .funct3(funct3_ex), .zero_flag(zero_flag_ex),
        .less_than(less_than_ex), .less_than_u(less_than_u_ex), .taken(branch_taken_ex)
    );
    ...
    always @(posedge clk or posedge rst) begin
        if (rst) begin
            regwrite_ex_mem <= 1'b0; memread_ex_mem <= 1'b0; memwrite_ex_mem <= 1'b0;
            memtoreg_ex_mem <= 1'b0; jump_ex_mem <= 1'b0; alu_result_ex_mem <= 32'h0;
            write_data_ex_mem <= 32'h0; pc_plus_4_ex_mem <= 32'h0; rd_ex_mem <= 5'b0;
            byte_size_ex_mem <= 2'b0; funct3_ex_mem <= 3'b0;
    ...
    LSU lsu_unit (
        .clk(clk), .rst(rst), .req_valid(lsu_req_valid), .req_ready(lsu_req_ready),
        .req_addr(alu_result_mem), .req_wdata(write_data_mem), .req_wstrb(lsu_req_wstrb),
        .req_is_load(memread_mem), .req_rd(rd_mem), .req_funct3(funct3_mem),
        .result_valid(lsu_result_valid), .result_data(lsu_result_data), .result_rd(lsu_result_rd),
        .result_ack(lsu_result_ack), .scoreboard(lsu_scoreboard), .dcache_req(dcache_req),
        .dcache_we(dcache_we), .dcache_addr(dcache_addr), .dcache_wdata(dcache_wdata),
        .dcache_wstrb(dcache_wstrb), .dcache_rdata(dcache_rdata), .dcache_ready(dcache_ready)
    );

    // ========================================================================
    // MEM/WB REGISTER (Replicated jump_mem_wb)
    // ========================================================================
    reg regwrite_mem_wb, memtoreg_mem_wb;
    // REPLICATION: jump_mem_wb split into 2 to reduce fanout
    reg jump_mem_wb_rep1, jump_mem_wb_rep2;
    ...
    always @(posedge clk or posedge rst) begin
        if (rst) begin
            ...
            jump_mem_wb_rep1 <= 1'b0; jump_mem_wb_rep2 <= 1'b0;
            alu_result_mem_wb <= 32'h0; mem_data_mem_wb <= 32'h0; pc_plus_4_mem_wb <= 32'h0;
            rd_mem_wb <= 5'b0;
        end else begin
            if (lsu_result_valid && !stall_if) begin
                ...
                jump_mem_wb_rep1  <= 1'b0;
                jump_mem_wb_rep2  <= 1'b0;
            end else if (!stall_any) begin
                ...
                jump_mem_wb_rep1  <= jump_mem;
                jump_mem_wb_rep2  <= jump_mem;
                rd_mem_wb         <= rd_mem;
            end
        end
    end
    ...
    assign jump_wb = jump_mem_wb_rep1;    assign alu_result_wb = alu_result_mem_wb;
    assign mem_data_wb = mem_data_mem_wb; assign pc_plus_4_wb = pc_plus_4_mem_wb;
    assign rd_wb = rd_mem_wb;

    // ========================================================================
    // STAGE 5: WB - Applied Replicated Jump Signal
    // ========================================================================
    wire [31:0] wb_data_before_jump;
    assign wb_data_before_jump = memtoreg_wb ? mem_data_wb : alu_result_wb;

    // REPLICATION USE: Divide 32-bit mux control to reduce capacitance load
    assign write_back_data_wb[15:0]  = jump_mem_wb_rep1 ? pc_plus_4_wb[15:0]  : wb_data_before_jump[15:0];
    assign write_back_data_wb[31:16] = jump_mem_wb_rep2 ? pc_plus_4_wb[31:16] : wb_data_before_jump[31:16];

    // ========================================================================
    // HAZARD DETECTION UNIT
    // ========================================================================
    hazard_detection hazard_unit (
        .memread_id_ex(memread_ex), .rd_id_ex(rd_ex), .rs1_id(rs1_id), .rs2_id(rs2_id),
        .memread_mem(memread_mem), .rd_mem(rd_mem), .branch_taken(pc_src_ex),
        .imem_ready(imem_ready), .lsu_scoreboard(lsu_scoreboard), .stall(stall),
        .stall_if(stall_if), .flush_if_id(flush_if_id), .flush_id_ex(flush_id_ex)
    );
