...
localparam [31:0] SIG0_VALUE      = 32'hD00D_CAFE;
localparam [31:0] SIG1_VALUE      = 32'h1357_9BDF;
localparam [31:0] RESULT_RUNNING  = 32'hCAFE_0001;
localparam [31:0] RESULT_FAIL     = 32'hDEAD_0001;
localparam integer HEARTBEAT_MIN  = 16;
localparam integer DC_REQ_MIN     = 8;
...
soc_hs #(
    .SIM_MODE(1),
    .IMEM_INIT_FILE("gnu_toolchain/tests/test_dualcore_basic.hex")
) chip (
    .clk_in      (clk),
    .por_n       (por_n),
    .ext_rst_n   (ext_rst_n),
...
always @(posedge clk) begin
    ...
    if (!prev_core0_dc_req && core0_dc_req) begin
        core0_dc_req_count <= core0_dc_req_count + 1;
    end
    if (!prev_core1_dc_req && core1_dc_req) begin
    ...
    if (dmem_result == RESULT_FAIL) begin
        $display("[FAIL] firmware reported RESULT_FAIL at cycle %0d", cycles);
        $finish;
    end

    if (!pass_reported &&
        dmem_sig0 == SIG0_VALUE &&
        dmem_sig1 == SIG1_VALUE &&
        dmem_result == RESULT_RUNNING &&
        dmem_heartbeat >= HEARTBEAT_MIN &&
        core0_dc_req_count >= DC_REQ_MIN &&
        core1_dc_req_count >= DC_REQ_MIN &&
        dcache0_writes != 0 &&
        dcache1_writes != 0 &&
        (dcache0_hits != 0 || dcache1_hits != 0)) begin
        pass_reported <= 1'b1;
        $display("[PASS] dual-core basic");
    ...
    if (cycles > TIMEOUT_CYCLES) begin
        $display("[FAIL] timeout waiting for dual-core progress");
        $display("  sig0=%08x sig1=%08x result=%08x heartbeat=%0d count=%0d",
                 dmem_sig0, dmem_sig1, dmem_result, dmem_heartbeat, dmem_count);
        $display("  core0_dc_req_count=%0d core1_dc_req_count=%0d", core0_dc_req_count, core1_dc_req_count);
        $display("  dcache0 writes=%0d hits=%0d", dcache0_writes, dcache0_hits);
        $display("  dcache1 writes=%0d hits=%0d", dcache1_writes, dcache1_hits);
        $finish;
