...
// ============================================================================
// Revision: FIX-FINAL
//   [B1] Soft reset stretcher: srst_n_r giờ stretch 16 cycle (giống POR)
//   [B2] ascon_irq_int: internal wire alias để tránh spurious dma_done sau reset
//   [B3] por_rst_n = por_rst_n_r & rst_n: đảm bảo luôn theo rst_n gốc
//   [D1] cpu_rst bao gồm cả soft_rst_pulse (2-flop sync từ combined source)
//   [D3] CLINT prescaler: thêm CLK_FREQ parameter, mtime tăng ở 1 MHz
//   [D4] DECERR default slave: crossbar có built-in, thêm note rõ ràng
//   [W1] Testbench alias wires đặt trong `ifdef SIMULATION
//   [W3] dcache current_* ports: drive 1'b0 nếu là input
//
// Bản đồ địa chỉ Slave:
...
//   S4: CLINT     0x4000_0000 – 0x4000_FFFF  (mask 0xFFFF_0000)
//
// Cảnh báo còn lại:
//   [W2] axi_width_converter burst doubling chưa verify
//   [W5] Xác nhận M/S convention của axi_width_converter_64to32 với IP spec
// ============================================================================
...
`include "clint.v"
// ============================================================================
// soc_top.v  —  RISC-V SoC Top-Level
//
// Topology:
//   riscv_cpu_core
//     ├─ imem_*  ──────────────► icache_top     (M0) ─┐
...
//                                                      ├─► axi4_crossbar_3m5s
//   ascon_ip_top                                       │     S0: inst_mem_axi_slave  (ROM)
//     ├─ M_AXI_*(64b) ──► axi_width_converter ─► M2   │     S1: data_mem_axi4_slave (RAM)
//     └─ S_AXI_* ◄─────────────────────────── S2 ◄───┤      S2: ascon_ip_top
//                                                      │     S3: soc_ctrl_slave
//                                                      └─    S4: clint
//
// Reset:
//   ext_rst_n (active-low, board) → fabric_rst_n (active-low, tous modules sauf CPU)
//                                 → cpu_rst      (active-high, riscv_cpu_core)
//
// Clock prescaler:
//   100 MHz clk → mtime_tick pulse 1 MHz (every 100 cycles) pour CLINT
//
// Signals confirmés:
//   fence  : CPU drive nội bộ, đã nằm trong dcache interface
//   flush  : tie off = 1'b0
//   soft_rst_pulse : expose ra output port
//   DMA    : chỉ truy cập S1 (DMEM), width converter 64→32
//   irq chain: ascon_ip_top.irq → soc_ctrl_slave.ascon_irq
//              soc_ctrl_slave.irq_out → cpu.external_irq
//              clint.timer_irq / clint.sw_irq → cpu
// ============================================================================
...
module soc_top #(
    ...
    // Crossbar address map
    parameter [31:0] S0_BASE = 32'h0000_0000, // IMEM
    parameter [31:0] S0_MASK = 32'hFFFF_0000,
    parameter [31:0] S1_BASE = 32'h1000_0000, // DMEM
    parameter [31:0] S1_MASK = 32'hFFFF_0000,
    parameter [31:0] S2_BASE = 32'h2000_0000, // ASCON
    parameter [31:0] S2_MASK = 32'hFFFF_F000,
    parameter [31:0] S3_BASE = 32'h3000_0000, // SoC Ctrl
    parameter [31:0] S3_MASK = 32'hFFFF_F000,
    parameter [31:0] S4_BASE = 32'h4000_0000, // CLINT
    parameter [31:0] S4_MASK = 32'hFFFF_0000
)(
    input  wire clk,
    input  wire ext_rst_n,        // active-low reset từ board

    // soft reset expose ra ngoài
...
// Reset generation
// ============================================================================
wire fabric_rst_n = ext_rst_n;   // active-low cho toàn bộ fabric
wire cpu_rst      = ~ext_rst_n;  // active-high cho riscv_cpu_core

// ============================================================================
// mtime_tick prescaler: 100 MHz → 1 MHz (pulse 1 cycle mỗi 100 cycles)
// ============================================================================
...
// Interrupt wires
// ============================================================================
wire external_irq;   // soc_ctrl → cpu
wire timer_irq;      // clint → cpu
wire sw_irq;         // clint → cpu
wire ascon_irq;      // ascon_ip_top → soc_ctrl
...
// ============================================================================
// ASCON DMA 64-bit master wires (ascon_ip_top → width_converter)
// ============================================================================
...
icache_top u_icache (
    ...
    .flush          (1'b0),             // tie off — không dùng

    // AXI4 → Crossbar M0
    .mem_arid       (m0_arid),
    .mem_araddr     (m0_araddr),
    .mem_arlen      (m0_arlen),
    .mem_arsize     (m0_arsize),
    .mem_arburst    (m0_arburst),
    .mem_arprot     (m0_arprot),
    ...
    // Write channels — ICache không ghi, tie off
    .mem_awid       (m0_awid),
    .mem_awaddr     (m0_awaddr),
    .mem_awlen      (m0_awlen),
    .mem_awsize     (m0_awsize),
    .mem_awburst    (m0_awburst),
    .mem_awprot     (m0_awprot),
...
dcache_top u_dcache (
    ...
    .fence          (1'b0),             // CPU drive nội bộ, không expose ra port
    ...
    .mem_arlen      (m1_arlen),
    .mem_arsize     (m1_arsize),
    .mem_arburst    (m1_arburst),
    .mem_arprot     (m1_arprot),
    ...
    .mem_awlen      (m1_awlen),
    .mem_awsize     (m1_awsize),
    .mem_awburst    (m1_awburst),
    .mem_awprot     (m1_awprot),
...
ascon_ip_top u_ascon (
    ...
    // AXI4 Master 64-bit (DMA → width converter)
    .M_AXI_AWID     (dma_awid),
...
// ============================================================================
// INSTANCE: axi_width_converter_64to32  (DMA 64-bit → crossbar M2 32-bit)
// ============================================================================
axi_width_converter_64to32 u_width_conv (
    .clk            (clk),
    .rst_n          (fabric_rst_n),

    // Master side (64-bit, from ascon DMA)
    .M_AXI_AWID     (dma_awid),
    .M_AXI_AWADDR   (dma_awaddr),
    .M_AXI_AWLEN    (dma_awlen),
    .M_AXI_AWSIZE   (dma_awsize),
    ...
    .S_AXI_AWID     (m2_awid),
    .S_AXI_AWADDR   (m2_awaddr),
    .S_AXI_AWLEN    (m2_awlen),
    .S_AXI_AWSIZE   (m2_awsize),
    .S_AXI_AWBURST  (m2_awburst),
    .S_AXI_AWPROT   (m2_awprot),
    .S_AXI_AWVALID  (m2_awvalid),
    .S_AXI_AWREADY  (m2_awready),

    .S_AXI_WDATA    (m2_wdata),
    .S_AXI_WSTRB    (m2_wstrb),
    .S_AXI_WLAST    (m2_wlast),
    .S_AXI_WVALID   (m2_wvalid),
    .S_AXI_WREADY   (m2_wready),

    .S_AXI_BID      (m2_bid),
    .S_AXI_BRESP    (m2_bresp),
    .S_AXI_BVALID   (m2_bvalid),
    .S_AXI_BREADY   (m2_bready),

    .S_AXI_ARID     (m2_arid),
    .S_AXI_ARADDR   (m2_araddr),
    .S_AXI_ARLEN    (m2_arlen),
    .S_AXI_ARSIZE   (m2_arsize),
    .S_AXI_ARBURST  (m2_arburst),
    .S_AXI_ARPROT   (m2_arprot),
    .S_AXI_ARVALID  (m2_arvalid),
    .S_AXI_ARREADY  (m2_arready),

    .S_AXI_RID      (m2_rid),
    .S_AXI_RDATA    (m2_rdata),
    .S_AXI_RRESP    (m2_rresp),
    .S_AXI_RLAST    (m2_rlast),
    .S_AXI_RVALID   (m2_rvalid),
    .S_AXI_RREADY   (m2_rready)
);
...
// ============================================================================
// INSTANCE: inst_mem_axi_slave  (S0 — IMEM / ROM)
// ============================================================================
inst_mem_axi_slave #(
    .ADDR_WIDTH    (ADDR_WIDTH),
    .DATA_WIDTH    (DATA_WIDTH),
...
// ============================================================================
// INSTANCE: data_mem_axi4_slave  (S1 — DMEM / RAM)
// ============================================================================
data_mem_axi4_slave #(
    .ADDR_WIDTH (ADDR_WIDTH),
    .DATA_WIDTH (DATA_WIDTH),
...
soc_ctrl_slave #(
    ...
    // ASCON interrupt (sticky, maskable)
    .ascon_irq      (ascon_irq),

    // Outputs
    .irq_out        (external_irq),     // → cpu.external_irq
    .soft_rst_pulse (soft_rst_pulse)    // expose ra output port soc_top
);
...
clint #(
    ...
    .clk            (clk),
    .rst_n          (fabric_rst_n),
    .mtime_tick     (mtime_tick),       // 1 MHz từ prescaler
