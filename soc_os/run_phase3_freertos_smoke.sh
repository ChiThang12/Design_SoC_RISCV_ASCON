#!/usr/bin/env bash
# Phase 3 minimal FreeRTOS-port smoke runner.
...
TEST_NAME="test_freertos_smoke"
HEX_PATH="$ROOT_DIR/gnu_toolchain/test_os/${TEST_NAME}.hex"
VVP_PATH="/tmp/${TEST_NAME}_phase3.vvp"
LOG_PATH="$ROOT_DIR/log/${TEST_NAME}.log"
UART_DIV=16

echo "=============================================="
echo " Phase 3 FreeRTOS minimal port smoke"
echo "=============================================="
...
if grep -q '\*\*\* PASS.*freertos_smoke' "$LOG_PATH"; then
    echo "  RESULT: PASS"
    grep '\*\*\* PASS.*freertos_smoke' "$LOG_PATH" | tail -n 1
    exit 0
fi

echo "  RESULT: FAIL/TIMEOUT"
grep -E 'CLINT] TIMER_IRQ|TEST-RESULT|Final PC|timer_irq|Message:|DECERR|WATCHDOG|STOP:|FreeRTOS|freertos_smoke' "$LOG_PATH" | tail -n 100 || true
exit 1
