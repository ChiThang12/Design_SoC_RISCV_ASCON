...
`ifndef IMEM_INIT_FILE
  `define IMEM_INIT_FILE "gnu_toolchain/tests/test_ascon_dma_fence_128b.hex"
`endif
...
module run_soc_fence_128b;
    ...
    task print_summary;
        begin
            ...
            $display("completion source        : internal dma_done or mailbox PASS");
    ...
    initial begin
        ...
        while (cycle_count < `TIMEOUT) begin
            ...
            if (result_mailbox !== RET_RUNNING && result_mailbox !== 32'h0) begin
                ...
                if (snoop_read_req_cnt != 0) fail("snoop read request observed in software-fenced baseline");
                if (snoop_read_hit_cnt != 0) fail("snoop read hit observed in software-fenced baseline");
                if (m2_ar_cnt < 1) fail("DMA did not issue AXI read in software-fenced baseline");
                if (m2_aw_cnt < 1) fail("DMA did not write output");

                print_summary;
                $display("[PASS] SoC software-fenced DMA 128B test passed");
                $finish;
            end
        end
