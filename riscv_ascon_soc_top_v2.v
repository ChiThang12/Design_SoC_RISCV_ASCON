// ============================================================================
// File: riscv_ascon_soc_top_v2.v
// ============================================================================
// Top-level SoC v2: sử dụng axi4_crossbar_3m4s (3 Master × 4 Slave)
//
// Thay đổi so với v1:
//   [1] Dùng axi4_crossbar_3m4s thay vì axi4_crossbar (2M→3M)
//   [2] DMA (M2) kết nối trực tiếp vào crossbar — XÓA DMEM arbiter thủ công
//   [3] S2 address decoder sửa lỗi: dùng AR sel register riêng để đảm bảo
//       R channel luôn routing về đúng slave (fix race condition)
//   [4] S2 B-channel dùng cùng latch với W-channel (s2_wr_sel_dma_r)
//       thay vì latch riêng s2_b_sel_dma_r (thực chất là cùng giá trị)
//   [5] DMA S_AXI_AWID / S_AXI_ARID được cung cấp (cũ bỏ trống)
//   [6] data_mem_axi4_slave được cấp S_AXI_ARID/AWID đầy đủ (cũ thiếu)
//   [7] ascon_CORE tag_received được gán từ register cục bộ (không thay đổi)
//
// Kiến trúc mới:
//
//   ┌─────────────────────────────────────────────────────────────────────┐
//   │                   riscv_ascon_soc_top_v2                            │
//   │                                                                     │
//   │  CPU ─ ICache(M0) ─┐                                               │
//   │                    ├─ axi4_crossbar_3m4s ─ S0: IMEM                │
//   │  CPU ─ DCache(M1) ─┤                     ─ S1: DMEM  ←── DMA(M2) │
//   │                    │                     ─ S2: ASCON+DMA cfg       │
//   │  DMA   (M2)  ──────┘                     ─ S3: SoC Ctrl           │
//   │                                                                     │
//   │  S2 (0x2000_0000)                                                   │
//   │   ├─ bit[8]=0 → ascon_axi_bridge (offset 0x000–0x0FF)             │
//   │   └─ bit[8]=1 → dma_top_axi4 S_AXI (offset 0x100–0x1FF)          │
//   └─────────────────────────────────────────────────────────────────────┘
// ============================================================================
...
module riscv_ascon_soc_top_v2 (
    ...
    // Interrupt outputs từ DMA
    output wire [3:0]  dma_irq_done,
    output wire [3:0]  dma_irq_error
);

    localparam ID_WIDTH = 4; // Phải >= 3 cho ID tagging 3-master
    ...
    // M2 (DMA — kết nối trực tiếp vào crossbar, không còn arbiter thủ công)
    ...
    // S1 (DMEM — DMA ghi/đọc trực tiếp qua crossbar S1)
    ...
    // ========================================================================
    // ── S2 Address Decoder (ASCON vs DMA config) ─────────────────────────────
    // Phân tách S2 (0x2000_0000 ~ 0x2000_01FF) thành 2 sub-slave:
    //   s2_addr[8] = 0 → ASCON bridge  (0x000–0x0FF)
    //   s2_addr[8] = 1 → DMA S_AXI cfg (0x100–0x1FF)
    //
    // FIX: Dùng latch riêng cho AR (s2_rd_sel_dma_r) để tránh glitch khi
    //      s2_araddr thay đổi giữa chừng. Tương tự cho AW/W/B.
    // ========================================================================
    wire s2_ar_sel_dma = s2_araddr[8];
    wire s2_aw_sel_dma = s2_awaddr[8];

    // AR channel: latch sel tại AR handshake
    reg  s2_rd_sel_dma_r;
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n)
            s2_rd_sel_dma_r <= 1'b0;
        else if (s2_arvalid && s2_arready)
            s2_rd_sel_dma_r <= s2_ar_sel_dma;
    end

    // --- ASCON bridge slave wires ---
    wire        ascon_s_arvalid, ascon_s_arready, ascon_s_rvalid, ascon_s_rlast;
    wire [31:0] ascon_s_rdata;
    wire [1:0]  ascon_s_rresp;
    wire [ID_WIDTH-1:0] ascon_s_rid;
    wire        ascon_s_awvalid, ascon_s_awready, ascon_s_wvalid, ascon_s_wready;
    wire        ascon_s_bvalid,  ascon_s_bready;
    wire [1:0]  ascon_s_bresp;
    wire [ID_WIDTH-1:0] ascon_s_bid;

    // --- DMA config slave wires ---
    wire        dma_s_arvalid, dma_s_arready, dma_s_rvalid, dma_s_rlast;
    wire [31:0] dma_s_rdata;
    wire [1:0]  dma_s_rresp;
    wire [ID_WIDTH-1:0] dma_s_rid;
    wire        dma_s_awvalid, dma_s_awready, dma_s_wvalid, dma_s_wready;
    wire        dma_s_bvalid,  dma_s_bready;
    wire [1:0]  dma_s_bresp;
    wire [ID_WIDTH-1:0] dma_s_bid;

    // AR routing
    assign ascon_s_arvalid = s2_arvalid & ~s2_ar_sel_dma;
    assign dma_s_arvalid   = s2_arvalid &  s2_ar_sel_dma;
    assign s2_arready      = s2_ar_sel_dma ? dma_s_arready : ascon_s_arready;

    // R channel mux (gated bằng latch từ AR handshake)
    ...
    // AW routing
    assign ascon_s_awvalid = s2_awvalid & ~s2_aw_sel_dma;
    assign dma_s_awvalid   = s2_awvalid &  s2_aw_sel_dma;
    assign s2_awready      = s2_aw_sel_dma ? dma_s_awready : ascon_s_awready;

    // W channel: latch sel tại AW handshake
    reg s2_wr_sel_dma_r;
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n)
            s2_wr_sel_dma_r <= 1'b0;
        else if (s2_awvalid && s2_awready)
            s2_wr_sel_dma_r <= s2_aw_sel_dma;
    end
    ...
    // B channel: dùng cùng latch với W (s2_wr_sel_dma_r)
    // Lưu ý: B response xảy ra sau W → latch vẫn đúng vì chưa thay đổi
    assign s2_bvalid      = s2_wr_sel_dma_r ? dma_s_bvalid  : ascon_s_bvalid;
    assign s2_bresp       = s2_wr_sel_dma_r ? dma_s_bresp   : ascon_s_bresp;
    assign s2_bid         = s2_wr_sel_dma_r ? dma_s_bid     : ascon_s_bid;
    assign ascon_s_bready = s2_bready & ~s2_wr_sel_dma_r;
    assign dma_s_bready   = s2_bready &  s2_wr_sel_dma_r;
    ...
    // M2 (DMA) được thêm vào — không còn arbiter thủ công bên ngoài nữa.
    // DMA chỉ nên truy cập S1 (DMEM); truy cập S0/S2/S3 sẽ nhận DECERR.
    // ========================================================================
    axi4_crossbar_3m4s #(.ID_WIDTH(ID_WIDTH)) xbar (
        ...
        // S1 — DMEM (DCache + DMA đều tới đây qua crossbar)
    ...
    // DMA nay truy cập DMEM qua crossbar (M2→S1), không cần arbiter bên ngoài.
    // data_mem_axi4_slave được cấp đầy đủ ARID/AWID từ crossbar.
    // ========================================================================
    data_mem_axi4_slave #(.ID_WIDTH(ID_WIDTH)) dmem (
        .clk           (clk),            .rst_n         (rst_n),
        .S_AXI_ARID    (s1_arid),
    ...
    // ========================================================================
    // ── 8. ASCON AXI Bridge ───────────────────────────────────────────────────
    // Register map (offset trong 4KB S2, bit[8]=0):
    //   0x00: CTRL      [0]=start (auto-clear), [1]=enc_dec
    //   0x04: MODE      [1:0]
    //   0x08: STATUS    [0]=busy, [1]=done (read-only)
    //   0x10–0x1C: KEY        (128-bit MSW first)
    ...
    //   0x80–0x8C: TAG_RECEIVED (write — for decryption verify)
    // ========================================================================
    reg         ascon_start_r;
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            ascon_start_r <= 1'b0; // auto-clear

            case (ab_wr_state)
                AB_WR_IDLE: begin
                    if (ascon_s_awvalid) begin
                        ab_wr_addr  <= s2_awaddr[7:0];
                        ab_bid_r    <= s2_awid;
                ...
                AB_WR_DATA: begin
                    if (ascon_s_wvalid) begin
                        case (ab_wr_addr)
                            8'h00: begin ascon_start_r <= s2_wdata[0]; ascon_enc_dec_r <= s2_wdata[1]; end
                            ...
                            8'h80: ascon_tag_received_r[127:96] <= s2_wdata;
                            8'h84: ascon_tag_received_r[95:64]  <= s2_wdata;
                            8'h88: ascon_tag_received_r[63:32]  <= s2_wdata;
                            8'h8C: ascon_tag_received_r[31:0]   <= s2_wdata;
                            default: ; // ignore
                        endcase
        ...
        if (!rst_n) begin
            ...
            if (ascon_s_arvalid && ascon_s_arready) begin
                ...
                case (s2_araddr[7:0])
                    ...
                    // FIX: thêm tag_match và tag_valid vào read map
                    8'h0C: ab_rdata_r <= {30'h0, ascon_tag_match, ascon_tag_valid};
    ...
    // ========================================================================
    // ── 10. DMA Controller ────────────────────────────────────────────────────
    // S_AXI ← CPU qua S2 decoder (offset 0x100–0x1FF)
    // M_AXI → M2 → crossbar → S1 (DMEM)
    // FIX: Thêm S_AXI_AWID / S_AXI_ARID (cũ để trống → warning)
    // ========================================================================
    dma_top_axi4 #(
        .ID_WIDTH  (ID_WIDTH),
        .DATA_WIDTH(32),
