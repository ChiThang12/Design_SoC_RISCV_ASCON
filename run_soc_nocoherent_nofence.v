...
`ifndef IMEM_INIT_FILE
  `define IMEM_INIT_FILE "gnu_toolchain/tests/test_ascon_dma_nocoherent_nofence.hex"
`endif
    ...
    initial begin
        ...
        while (cycle_count < `TIMEOUT) begin
            ...
            if (result_mailbox !== RET_RUNNING && result_mailbox !== 32'h0) begin
                if (result_mailbox !== RET_BADOUT)
                    fail("firmware did not report expected bad output");
                ...
                if (coh_ctrl[1:0] !== 2'b00) fail("coherence control unexpectedly enabled");
                if (snoop_read_req_cnt != 0) fail("snoop read request observed while coherence disabled");
                if (snoop_read_hit_cnt != 0) fail("snoop read hit observed while coherence disabled");
                if (saw_expected_snoop_data) fail("unexpected dirty data observed on snoop path");
                if (saw_expected_core_ptext) fail("DMA unexpectedly received dirty plaintext with coherence disabled");
                if (m2_ar_cnt < 1) fail("DMA did not fall back to AXI read with coherence disabled");
                if (m2_aw_cnt < 1) fail("DMA did not write output");

                print_summary;
                $display("[PASS] SoC no-coherent negative no-fence test passed (expected firmware RET_BADOUT)");
                $finish;
            end
        end
