...
module dma_channel #(
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            // WHY: done/error/rd_rel/wr_rel là 1-cycle pulse. Set trong state
            // cụ thể, default clear để tránh giữ HIGH nhiều cycle.
            done   <= 1'b0;
            error  <= 1'b0;
            rd_rel <= 1'b0;
            wr_rel <= 1'b0;
            ...
            case (state)
                ...
                S_IDLE: begin
                    if (cfg_start && cfg_en) begin
                        if (cfg_mode != 2'b00) begin
                            // [FIX] Mode khác mem-to-mem chưa hỗ trợ
                            error <= 1'b1;
                        end else if (cfg_len == 32'd0) begin
                            done <= 1'b1;
                        end else begin
                            cur_src    <= cfg_src;
                            cur_dst    <= cfg_dst;
                ...
                S_FETCH_WR: begin
                    if (wr_grant) begin
                        wr_addr  <= cur_dst;
                        // [FIX] Bug: buf_wr_ptr là reg[3:0], sau 16 beats wrap về 0.
                        // 0 - 1 trong context 8-bit = 8'hFF = 255, không phải 15.
                        // Dùng rd_len (đã lưu ARLEN cho burst này) thay vì buf_wr_ptr-1.
                        // WHY rd_len đúng: rd_len = next_burst_len = số beats - 1,
                        // chính xác là AWLEN cần thiết cho write burst tương ứng.
                        wr_len   <= rd_len;   // [FIX] thay buf_wr_ptr - 4'd1
                        wr_valid <= 1'b1;
                        wr_req   <= 1'b0;
                        state    <= S_WAIT_WR;
                    end
                ...
                S_WAIT_WR: begin
                    ...
                    if (wr_bvalid) begin
                        if (wr_bresp != 2'b00) begin
                            // Slave báo lỗi (SLVERR/DECERR)
                            error  <= 1'b1;
                            wr_rel <= 1'b1;
                            state  <= S_ERROR;
                        end else begin
                            // [FIX-3] Tính bytes đã transfer tường minh
                            cur_src    <= cur_src    + burst_bytes;
                            cur_dst    <= cur_dst    + burst_bytes;
                            bytes_left <= bytes_left - burst_bytes;
                            wr_rel     <= 1'b1;

                            // [FIX] bytes_left comparison dùng giá trị TRƯỚC non-blocking update.
                            // "bytes_left <= burst_bytes" đọc bytes_left cũ (trước phép trừ).
                            // Khi bytes_left == burst_bytes: sau khi trừ còn 0 → done.
                            // Khi bytes_left < burst_bytes: partial burst cuối → done.
                            // Khi bytes_left > burst_bytes: còn data → loop tiếp.
                            // Điều kiện đúng: bytes_left <= burst_bytes → done (không loop nữa).
                            // Đây là ĐÚNG vì non-blocking: if đọc bytes_left cũ, <= update sau.
                            // Bug thật sự là wr_len sai (AW#1 len=255) khiến burst_bytes=1024
                            // >> bytes_left=128 → điều kiện luôn đúng → done sau burst 1.
                            // Sau khi fix wr_len, điều kiện này sẽ tính đúng.
                            if (bytes_left <= burst_bytes) begin
                                // Transfer hoàn tất
                                state <= S_DONE;
                            end else begin
                                // Còn data → loop: read burst tiếp theo
                                buf_wr_ptr <= 4'd0;
                                buf_rd_ptr <= 4'd0;
                                buf_count  <= 5'd0;
                                rd_req     <= 1'b1;
