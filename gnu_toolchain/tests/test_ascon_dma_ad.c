/* test_ascon_dma_ad.c — DMA 16-block (128B) encrypt, 8B AD
 * Full AEAD pipeline — closest to JPEG camera use case. */
#include <stdint.h>
#include "uart.h"
#include "ascon.h"
...
/* AD buffer trong DMEM (free region sau CT_MULTI). Trong DMA mode, ASCON DMA
 * fetch AD từ memory tại AD_ADDR (ascon_axi_slave 0x120) — KHÔNG dùng thanh ghi
 * AD_DATA (đó là đường CPU-direct). Word order khớp register path:
 *   mem[AD_BUF+0] = AD_DATA_0 (MS word), mem[AD_BUF+4] = AD_DATA_1. */
#define AD_BUF_BASE  0x10000330UL
...
static int run_dma_ad_test(void)
{
    ...
    /* DMA-mode AD: ghi AD payload vào DMEM, trỏ AD_ADDR tới buffer (không dùng
     * ascon_set_ad register path — DMA fetch AD từ memory). */
    {
        volatile uint32_t * const adb = (volatile uint32_t *)AD_BUF_BASE;
        adb[0] = TV_AD_W0;
        adb[1] = TV_AD_W1;
        __asm__ volatile ("fence w,w" ::: "memory");
    }
    ASCON_WRITE(ASCON_OFS_AD_ADDR, (uint32_t)AD_BUF_BASE);
    ASCON_WRITE(ASCON_OFS_AD_LEN,  TV_AD_LEN);
    ...
    do {
        ...
        if (--timeout == 0u) {
            ...
            uart_puts("[FAIL] ascon_dma_ad timeout\r\n");
            return -1;
        }
    ...
    if (status & (ASCON_ST_DMA_ERR | ASCON_ST_CORE_ERR)) {
        ...
        uart_puts("[FAIL] ascon_dma_ad DMA_ERR\r\n");
        return -1;
    }
