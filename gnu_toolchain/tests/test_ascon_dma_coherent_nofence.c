/* test_ascon_dma_coherent_nofence.c
 * End-to-end hardware-coherent ASCON DMA smoke test.
...
#define PT_WORD0    0xA5A50001u
#define PT_WORD1    0x5A5A0002u
...
{
    volatile uint32_t *pt = (volatile uint32_t *)DMEM_DMA_SRC_ADDR;

    /* First allocate/fill the line, then overwrite with the data DMA must see.
     * No hardware fence follows these stores. */
    pt[0] = STALE_WORD0;
    pt[1] = STALE_WORD1;
    pt[0] = PT_WORD0;
    pt[1] = PT_WORD1;
...
int main(void)
{
    uint32_t status;
    uint32_t timeout;
    volatile uint32_t *ct = (volatile uint32_t *)DMEM_DMA_OUTPUT_ADDR;
    ...
    ascon_dma_config(DMEM_DMA_SRC_ADDR, DMEM_DMA_OUTPUT_ADDR, 8u);
    ...
    /* Start DMA+CORE with no hardware fence after dirty plaintext stores. */
