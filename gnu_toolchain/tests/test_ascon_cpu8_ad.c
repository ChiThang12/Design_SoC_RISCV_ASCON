/* test_ascon_cpu8_ad.c — CPU-direct 8B encrypt, 8B AD
 * Measures AD phase overhead vs no-AD baseline (T1). */
#include <stdint.h>
#include "uart.h"
#include "ascon.h"
...
static int run_cpu8_ad_test(void)
{
    ...
    ascon_set_ad(TV_AD_W0, TV_AD_W1, 0u, 0u, TV_AD_LEN);
    ...
    do {
        ...
        if (--timeout == 0u) {
            ...
            uart_puts("[FAIL] ascon_cpu8_ad timeout\r\n");
            return -1;
        }
    ...
    if (status & ASCON_ST_CORE_ERR) {
        ...
        uart_puts("[FAIL] ascon_cpu8_ad CORE_ERR\r\n");
        return -1;
    }
